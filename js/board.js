...
export default class Board {
	...
	initBoard() {
		//init board blocks
		for(var i = 0, x = Sprite.SIDE_LR_WIDTH; i < this.numBlocksX; i++, x += Block.SIZE) {
			for(var j = 0, y = (2 * Sprite.SIDE_TMD_HEIGHT) + Sprite.PANEL_HEIGHT; j < this.numBlocksY; j++, y += Block.SIZE) {
		...
		for(let i = 0; i < this.numBlocksX; i++) {
			for(let j = 0; j <this.numBlocksY ; j++) {
				...
				if(mouseX > blockX && mouseX < blockX + Block.SIZE && mouseY > blockY && mouseY < blockY + Block.SIZE) {
					...
					else if(mouse == "left" && !this.board[i][j].getFlagged) {
						// uncover block
	...
	draw(image, ctx) {
		for(let i = 0; i < this.numBlocksX; i++) {
			for(let j = 0; j < this.numBlocksY; j++) {
				// mine
				if(this.board[i][j].getIsMine && !this.board[i][j].getCovered)		
					ctx.drawImage(image, 124, 0, 24, 24, this.board[i][j].getX, this.board[i][j].getY, 24, 24);
				// flagged 
				else if(this.board[i][j].getFlagged)		
					ctx.drawImage(image, 48, 0, 24, 24, this.board[i][j].getX, this.board[i][j].getY, 24, 24);
				// covered
				else if(this.board[i][j].getCovered)
					ctx.drawImage(image, 0, 0, 24, 24, this.board[i][j].getX, this.board[i][j].getY, 24, 24);
				// if block not covered draw his number
				else if(!this.board[i][j].getCovered && this.board[i][j].getMinesAround >= 0) {
					switch(this.board[i][j].getMinesAround) {
						case 0: 
							ctx.drawImage(image, 24, 0, 24, 24, this.board[i][j].getX, this.board[i][j].getY, 24, 24);
							break;
						case 1: 
							ctx.drawImage(image, 193, 0, 24, 24, this.board[i][j].getX, this.board[i][j].getY, 24, 24);
							break;
						case 2: 
							ctx.drawImage(image, 217, 0, 24, 24, this.board[i][j].getX, this.board[i][j].getY, 24, 24);
							break;
						case 3: 
							ctx.drawImage(image, 241, 0, 24, 24, this.board[i][j].getX, this.board[i][j].getY, 24, 24);
							break;
						case 4: 
							ctx.drawImage(image, 265, 0, 24, 24, this.board[i][j].getX, this.board[i][j].getY, 24, 24);
							break;
						case 5: 
							ctx.drawImage(image, 289, 0, 24, 24, this.board[i][j].getX, this.board[i][j].getY, 24, 24);
							break;
						case 6: 
							ctx.drawImage(image, 303, 0, 24, 24, this.board[i][j].getX, this.board[i][j].getY, 24, 24);
							break;
						case 7: 
							ctx.drawImage(image, 327, 0, 24, 24, this.board[i][j].getX, this.board[i][j].getY, 24, 24);
							break;
						case 8: 
							ctx.drawImage(image, 341, 0, 24, 24, this.board[i][j].getX, this.board[i][j].getY, 24, 24);
							break;
